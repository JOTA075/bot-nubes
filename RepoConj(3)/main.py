...
tl_admin_users = ['STANISLASKI','DRP96','Orisha91'] #Poner aqui los user con acceso permanente
godlist = ['STANISLASKI','DRP96','Orisha91'] #Poner aqui los admin 
...
async def onmessage(bot:TelegramClient,ev: NewMessage.Event,loop,ret=False):
    ...
    if username not in tl_admin_users:
        await bot.send_message(ev.chat.id,'❌ ℕ𝕠 𝕥𝕚𝕖𝕟𝕖𝕤 #𝕒𝕔𝕔𝕖𝕤𝕠, 𝕔𝕠𝕟𝕥𝕒𝕔𝕥𝕒 𝕔𝕠𝕟 @STANISLASKI ❌')
        return
    ...
    if '/up' in text:
        ...
        try:
            ...
            await bot.send_file(ev.chat,txtsendname,
                                caption=f'{txtinfo}',
                                thumb='thumb.png',
                                buttons=[Button.url('|ıllıll Ɇł Ᵽɍøfɇsøɍ |ıllıllı','https://t.me/STANISLASKI')])
